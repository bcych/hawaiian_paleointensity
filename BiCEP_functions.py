...
class SpecimenCollection():
    ...
    def histplot(self,ax,**kwargs):
        ...
        **kwargs:
        ...
        None
        """
        ax.hist(extract_values(self.fit,'int_site'),bins=100,color='skyblue',density=True,**kwargs)
...
class Specimen():
    ...
    def plot_arai(self,ax,temps=True):
        ...
        clusters=KMeans().fit_predict(self.IZZI.loc[:,['PTRM','NRM']])
        indices=[]
        ...
        for j in indices:
            ax.text(self.IZZI.PTRM.iloc[j]/self.NRM0,self.IZZI.NRM.iloc[j]/self.NRM0,str(int(self.temps[j]-273)),alpha=0.5)
    ...
    def plot_zijd(self,ax,temps=True):
        ...
        if temps==True:
            shared_axis=0
            zrange=np.max(self.NRM_dirs[:,2])-np.min(self.NRM_dirs[:,2])
            yrange=np.max(self.NRM_dirs[:,1])-np.min(self.NRM_dirs[:,1])
            if yrange>zrange:
                textindex=1
            else:
                textindex=2
            grads=np.gradient(self.NRM_dirs,axis=0)
            gradgrads=np.gradient(grads,axis=0)

            norms=-1/grads
            norms[norms==-np.inf]=-1e38
            norms[norms==np.inf]=1e38
            norms=np.transpose(norms.T/np.linalg.norm(norms[:,np.array([shared_axis,textindex])],axis=1))
            dots=np.array(list(map(np.dot,grads,gradgrads)))
            signs=-np.sign(dots)
        
            


            clusters=KMeans().fit_predict(self.NRM_dirs)
            ...
            for j in indices:
                ...
                if diff[textindex]>0:
                    ...
                    va='top'
                ax.text(self.NRM_dirs[j,shared_axis],self.NRM_dirs[j,textindex],str(int(self.temps[j]-273)),alpha=0.5,va=va,ha=ha)

        
        #if temps==True:
            #for i in range(len(self.temps)):
                #ax.text(self.NRM_dirs[i,0],-self.NRM_dirs[i,2],str(int(self.temps[i]-273)),alpha=0.5)
